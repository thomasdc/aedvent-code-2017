...
func main(){
	//part 1
	fmt.Println( Checksum(&example1) )
	fmt.Println(
		Checksum( parser(input) ),
	)

	//part 2
	fmt.Println( Checksum2(&example2) )
	fmt.Println(
		Checksum2( parser(input) ),
	)
...
func Checksum(sheetPtr *[][]int) int{
...
func Checksum2(sheetPtr *[][]int) int{
...
func findDivisible(line *[]int) int{
	row := *line
	...
	for i := 0 ; i < len(row) ; i++ {
		for j := i+1 ; j < len(row) ; j++ {
			...
			if row[i] <= row[j] && row[j] % row[i] == 0 {
				return row[j] / row[i]
			} else if row[i] > row[j] && row[i] % row[j] == 0 {
				return row[i] / row[j]
			}
