...
namespace Checksum
{
    public static class Program
    {
        ...
        {
            Console.WriteLine(Part2(@"4168	3925	858	2203	440	185	2886	160	1811	4272	4333	2180	174	157	361	1555
...
37	1069	166	1037	172	258	1071	90	497	1219	145	1206	143	153	1067	510"));
        ...
        public static int Part1(string input)
        {
            ...
            return parsedInput.Select(_ => _.Max() - _.Min()).Sum();
        }
        ...
        public static int Part2(string input)
        {
            ...
            for (var i = 0; i < parsedInput.Length; i++)
            {
                ...
                {
                    var value = parsedInput[i][j];
                    for (var k = j+1; k < parsedInput[i].Length; k++)
                    {
                        var otherValue = parsedInput[i][k];
                        var division = (double) Math.Max(value, otherValue) / Math.Min(value, otherValue);
                        if (division % 1 == 0)
                        {
                            sum += Convert.ToInt32(division);
                        }
                    }
                }
            }
        ...
        private static int[][] Parse(string input)
        {
            ...
            {
                result[i] = splitted[i].Split("\t").Select(int.Parse).ToArray();
            }
