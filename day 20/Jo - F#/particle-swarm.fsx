...
let example = "p=<3,0,0>, v=<-1,0,0>, a=<-1,0,0>\np=<-8,0,0>, v=<-6,0,0>, a=<-2,0,0>"
...
//Naively running takes a loooong time, just look for the particle with least acceleration.
#time
//part1 1 parsed
parsed 
|> List.map (fun p -> length p.Acceleration, p)
|> List.sortBy fst
